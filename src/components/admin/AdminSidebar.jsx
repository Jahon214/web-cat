...
const AdminSidebar = () => {
  ...
  return (
    <>
          ...
          {menuItems.map((item) => (
            <Link
              ...
              className={`flex items-center gap-4 px-5 py-4 rounded-2xl transition-all duration-300 group ${
                isActive(item.path)
                  ? "bg-linear-to-r from-purple-600 to-indigo-600 text-white shadow-xl shadow-purple-900/40 translate-x-2"
      ...
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-100 bg-[#0f172a] h-70 flex justify-around items-center px-4 rounded-t-[30px] border-t border-white/5 shadow-[0_-10px_40px_rgba(0,0,0,0.4)]">
        {menuItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <div className="absolute -top-7.5 w-14 h-14 bg-linear-to-tr from-purple-600 to-indigo-600 rounded-full border-[5px] border-[#f8fafc] shadow-lg shadow-purple-900/40 flex items-center justify-center transition-all duration-500 animate-pop">

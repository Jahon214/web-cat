...
const StatCard = ({ item }) => {
  ...
  return (
    <div className="group relative bg-white p-7 rounded-[35px] shadow-sm border border-gray-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 overflow-hidden">
      <div className={`absolute -right-6 -top-6 w-24 h-24 bg-linear-to-br ${config.color} opacity-5 group-hover:opacity-10 group-hover:scale-150 transition-all duration-700 rounded-full`}></div>
      <div className="relative z-10">
        <div className={`w-14 h-14 rounded-2xl bg-linear-to-br ${config.color} flex items-center justify-center text-white text-2xl shadow-lg mb-6 group-hover:rotate-6 transition-transform`}>
          {config.icon}
        </div>
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        {/* CHART CONTAINER */}
        <div className="lg:col-span-2 bg-linear-to-br from-gray-900 via-gray-900 to-purple-950 rounded-[50px] p-10 shadow-2xl relative overflow-hidden">
          ...
          <div className="h-56 w-full flex items-end gap-5 justify-around relative z-10">
            {DASHBOARD_DATA.chartData.map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-4 group/bar h-full justify-end">
                <div 
                  style={{ height: `${val}%` }} 
                  className="w-full bg-linear-to-t from-purple-700 via-indigo-600 to-purple-400 rounded-t-2xl opacity-70 group-hover/bar:opacity-100 group-hover/bar:scale-x-110 transition-all duration-500 relative cursor-pointer"
                >
